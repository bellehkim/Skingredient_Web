...
import { mockSkinAnalysisService, ANALYSIS_STEPS } from "@/lib/skinAnalysisService";
...
function Scan() {
  ...
  const start = async () => {
    setScanning(true);
    setStep(0);
    const result = await mockSkinAnalysisService.analyze();
    setAnalysis(result);
  ...
  return (
    <AppShell title="Skin Scan" back="/">
      <MobileHeader
        ...
        back="/"
        rightSlot={
          <Link to="/" aria-label="Help" className="grid h-9 w-9 place-items-center rounded-full text-ink hover:bg-surface-muted">
            <HelpCircle size={20} />
          </Link>
        }
      />
      <div className="px-5 lg:px-10 lg:pt-8">
        ...
        <div className="relative mx-auto mt-2 aspect-[3/4] w-full overflow-hidden rounded-[28px] bg-gradient-to-br from-[#F5EFE4] to-[#EDDDC8] lg:mt-0 lg:max-w-[440px]">
          ...
          {/* Instruction pill */}
          <div className="absolute left-1/2 top-4 -translate-x-1/2 rounded-full bg-black/60 px-4 py-1.5 text-[12px] font-medium text-white">
            {scanning ? ANALYSIS_STEPS[step] : "Position your face in the frame"}
          </div>
          ...
          <div className="absolute inset-x-0 bottom-0 space-y-3 bg-gradient-to-t from-black/40 to-transparent p-4 pt-8 text-white">
            ...
            <div className="flex items-center justify-between">
              <button aria-label="Upload photo" className="grid h-11 w-11 place-items-center rounded-full bg-white/90 text-ink">
                <ImageIcon size={18} />
              </button>
              <button
                onClick={start}
              ...
              >
                <div className={`h-14 w-14 rounded-full ${scanning ? "animate-pulse bg-sage" : "bg-white"}`} />
              </button>
              <button aria-label="Flash" className="grid h-11 w-11 place-items-center rounded-full bg-white/90 text-ink">
                <Zap size={18} />
              </button>
            </div>
          </div>
        ...
        <div className="hidden lg:block">
          ...
          <p className="mt-2 max-w-[440px] text-[14px] leading-relaxed text-ink-muted">
            Face a soft, even light source, remove glasses, and keep your whole face inside the frame.
            You can use your camera or upload a recent photo.
          </p>
          <ul className="mt-5 space-y-2 text-[13.5px] text-ink">
            {["Natural daylight works best", "No makeup for the most accurate read", "Hold still while we analyze"].map((t) => (
              <li key={t} className="flex items-center gap-2.5 rounded-2xl bg-surface-muted px-4 py-2.5">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            ...
            >
              {scanning ? "Analyzing…" : "Use camera"}
            </button>
            <button className="inline-flex items-center gap-2 rounded-2xl border border-hairline bg-white px-6 py-3 text-[14px] font-semibold text-ink">
        ...
        <div className="mt-4 flex items-center justify-between px-1 text-[13px] text-ink-muted lg:mt-8">
          <Link to="/scan/check-in" className="font-medium text-brand lg:hidden">Daily check-in</Link>
...
function FaceIllustration() {
  return (
    <svg viewBox="0 0 200 260" className="h-full w-full opacity-95">
      ...
      <path d="M85 165 Q100 175 115 165" stroke="#C77A6A" strokeWidth="3" fill="none" strokeLinecap="round" />
    </svg>
  );
}
...
function MeshOverlay() {
  const lines = [];
  for (let i = 0; i < 10; i++) {
    lines.push(<line key={"h" + i} x1="0" y1={i * 30} x2="400" y2={i * 30} stroke="white" strokeOpacity="0.15" />);
    lines.push(<line key={"v" + i} x1={i * 30} y1="0" x2={i * 30} y2="500" stroke="white" strokeOpacity="0.15" />);
  }
  return (
