...
function ProductDetail() {
  ...
  const { products, recordReaction } = useAppStore();
  ...
  return (
    ...
    >
      <MobileHeader
        ...
        back="/shelf"
        rightSlot={
          <button aria-label="Favorite" onClick={() => setFav((v) => !v)} className="grid h-9 w-9 place-items-center rounded-full">
            <Heart size={20} className={fav ? "text-coral" : "text-ink-muted"} fill={fav ? "currentColor" : "none"} />
          </button>
        }
      />
      <PageContainer>
        ...
        <div className="mt-5 lg:mt-0">
          ...
          <h1 className="mt-1 text-[24px] font-bold leading-tight text-ink lg:text-[28px]">{product.name}</h1>
          ...
          {product.benefitTags && product.benefitTags.length > 0 && (
            <ul className="mt-3 space-y-2">
              {product.benefitTags.map((b) => (
                <li key={b} className="flex items-center gap-2.5 rounded-2xl bg-surface-muted px-3.5 py-2.5">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  product.keyIngredients.forEach((i) =>
                    recordReaction(
                      i,
                      r === "Helpful" ? "helpful" : r === "Neutral" ? "neutral" : r === "Irritating" ? "irritating" : "unknown",
                    ),
                  );
                }}
                className={`rounded-xl border py-2.5 text-[13px] font-medium ${
                  reaction === r ? "border-brand bg-brand text-white" : "border-hairline bg-white text-ink"
                }`}
              >
                {r}
              </button>
