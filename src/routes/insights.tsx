      ...
      <PageContainer width="wide">
        ...
        <section className="rounded-3xl border border-hairline bg-white p-4 shadow-soft lg:p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-surface-muted px-2 py-1 text-[11px] font-medium text-ink-muted">7 days</span>
          </div>
          <div className="mt-3 h-48 lg:h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={mockRednessTrend} margin={{ top: 10, right: 6, left: -20, bottom: 0 }}>
                <defs>
                ...
                <XAxis dataKey="day" tickLine={false} axisLine={false} tick={{ fill: "#667085", fontSize: 11 }} />
                <YAxis domain={[0, 100]} tickLine={false} axisLine={false} tick={{ fill: "#667085", fontSize: 11 }} />
                <Tooltip cursor={{ stroke: "#e8ecf2" }} contentStyle={{ borderRadius: 12, border: "1px solid #e8ecf2", fontSize: 12 }} />
                <Area type="monotone" dataKey="value" stroke="#FF626A" strokeWidth={2.5} fill="url(#rd)" dot={{ r: 3, fill: "#FF626A" }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </section>
        ...
        <div className="grid grid-cols-3 gap-2 lg:grid-cols-1 lg:gap-3">
          <Metric name="Hydration" value={48} label="Low" ring="#2798F2" bg="#EAF6FF" />
          <Metric name="Acne" value={32} label="Low" ring="#45B887" bg="#E8F8F1" />
          <Metric name="Oiliness" value={52} label="Normal" ring="#F5B82E" bg="#FFF7D8" />
        </div>
        </div>
        ...
        >
          ...
          <Link to="/insights" className="mt-3 inline-flex items-center gap-1 text-[13px] font-semibold text-brand">
...
function Metric({ name, value, label, ring, bg }: { name: string; value: number; label: string; ring: string; bg: string }) {
  const r = 22;
  const c = 2 * Math.PI * r;
    ...
    <div className="rounded-3xl border border-hairline bg-white p-3 text-center shadow-soft">
      ...
      <div className="relative mx-auto mt-2 h-16 w-16">
        <svg viewBox="0 0 60 60" className="h-full w-full -rotate-90">
          ...
          <circle cx="30" cy="30" r={r} stroke={ring} strokeWidth="5" fill="none" strokeLinecap="round"
            strokeDasharray={c} strokeDashoffset={off} />
        </svg>
        <div className="absolute inset-0 grid place-items-center text-[15px] font-bold text-ink">{value}</div>
      </div>
      <p className="mt-1 text-[11px] font-semibold" style={{ color: ring }}>{label}</p>
    </div>
  );
}